...
$(document).ready(function() {
    ...
    document.querySelectorAll('.video-thumb').forEach(video => {
        video.addEventListener('click', () => {
            ...
            modalVideo.src = video.dataset.video;
            modalVideo.play();
        });
    });

    document.querySelector('.video-modal__close').addEventListener('click', closeModal);
    ...
    // **اضافه کردن بسته شدن منو هنگام کلیک روی آیتم**
    $('.nav__item').on('click', function() {
        // اگر منو در حال نمایش است
        if (sideNavTl.began && !sideNavTl.reversed) {
            sideNavTl.reverse();
            sideNavTl.completed = false;
            sideNavTl.play();
        }
