...
;document.addEventListener("DOMContentLoaded",function(){
  const laptopPreviews=document.querySelectorAll(".auto-laptop-scroll");
  function updateLaptopScroll(preview){const screen=preview.querySelector(".laptop-preview__screen"),image=preview.querySelector(".laptop-preview__screen img");if(!screen||!image)return;const distance=Math.max(0,image.getBoundingClientRect().height-screen.clientHeight);preview.style.setProperty("--scroll-distance",distance+"px");preview.style.setProperty("--scroll-duration",Math.max(16,Math.min(42,distance/45))+"s");preview.classList.toggle("no-scroll-needed",distance<8)}
  laptopPreviews.forEach(function(preview){const image=preview.querySelector(".laptop-preview__screen img");if(image&&image.complete)updateLaptopScroll(preview);else if(image)image.addEventListener("load",function(){updateLaptopScroll(preview)},{once:true})});
  window.addEventListener("resize",function(){laptopPreviews.forEach(updateLaptopScroll)},{passive:true});
  if("IntersectionObserver" in window){const observer=new IntersectionObserver(function(entries){entries.forEach(function(entry){updateLaptopScroll(entry.target);entry.target.classList.toggle("is-visible",entry.isIntersecting)})},{threshold:.35});laptopPreviews.forEach(function(preview){observer.observe(preview)})}else{laptopPreviews.forEach(function(preview){updateLaptopScroll(preview);preview.classList.add("is-visible")})}
  const lazyVideos=document.querySelectorAll('video.video-thumb source[data-src]');
  function loadVideo(video){if(video.dataset.loaded)return;video.querySelectorAll('source[data-src]').forEach(function(s){s.src=s.dataset.src});video.dataset.loaded='true';video.load();video.play&&video.play().catch(function(){})}
  if('IntersectionObserver' in window){const vobs=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting){loadVideo(entry.target);vobs.unobserve(entry.target)}})},{rootMargin:'250px 0px'});document.querySelectorAll('video.video-thumb').forEach(function(v){vobs.observe(v)})}else{document.querySelectorAll('video.video-thumb').forEach(loadVideo)}
});
